...
auto PlainUsernameValue(not_null<PeerData*> peer) -> rpl::producer<QString> {
	const auto &ayu = AyuSettings::getInstance();
	if (peer->isSelf()) {
		return rpl::combine(
			ayu.localProfileUsernameValue(),
			ayu.localProfileEditorValue(),
			rpl::merge(
			...
			| rpl::map([peer](const QString &local, bool enabled, auto) {
				if (enabled && !local.isEmpty()) {
					auto result = local;
...
rpl::producer<QString> NameValue(not_null<PeerData*> peer) {
	...
	if (peer->isSelf()) {
		return rpl::combine(
			...
			ayu.localProfileEditorValue(),
			peer->session().changes().peerFlagsValue(peer, UpdateFlag::Name))
			| rpl::map([peer](const QString &local, bool enabled, auto) {
				return enabled && !local.isEmpty()
					? local
					: peer->name();
			}) | rpl::type_erased;
	}
...
rpl::producer<TextWithEntities> PhoneValue(not_null<UserData*> user) {
	...
	if (user->isSelf()) {
		return rpl::combine(
			...
			| rpl::map([user](const QString &local, bool enabled, auto) {
				return tr::marked(enabled && !local.isEmpty()
					? local
...
rpl::producer<TextWithEntities> PhoneOrHiddenValue(not_null<UserData*> user) {
	return rpl::combine(
		...
		if (phone.text.isEmpty() && username.isEmpty() && about.isEmpty()) {
			...
			|| (user->isSelf()
				&& !AyuSettings::getInstance().localProfilePhone().isEmpty()
				&& AyuSettings::getInstance().localProfileEditor())) {
			const auto localPhone = user->isSelf()
				? AyuSettings::getInstance().localProfilePhone()
				: user->phone();
			auto phoneLink = localPhone;
			for (const auto ch : { ' ', '-', '(', ')' }) {
				phoneLink.remove(ch);
			}
			return tr::link(phone, u"internal:collectible_phone/"_q
				+ phoneLink + '@' + QString::number(user->id.value));
		} else {
			return phone;
		}
	});
...
rpl::producer<std::vector<TextWithEntities>> UsernamesValue(
	...
	if (peer->isSelf()) {
		return rpl::combine(
			...
			ayu.localAdditionalUsernamesValue(),
			ayu.localProfileEditorValue()) | rpl::map([peer](
				...
				if (!primary.isEmpty()) {
					result.push_back(tr::link(
						primary,
						UsernameUrl(peer, primary)));
				}
				for (const auto &entry : raw.split(',', Qt::SkipEmptyParts)) {
					...
					if (parts[0].trimmed().isEmpty()) continue;
					auto username = parts[0].trimmed();
...
rpl::producer<int> PeerGiftsCountValue(not_null<PeerData*> peer) {
	const auto &ayu = AyuSettings::getInstance();
	if (peer->isSelf() || ayu.localProfileEditor()) {
		return rpl::combine(
			peer->session().changes().peerFlagsValue(
				peer,
				UpdateFlag::PeerGifts),
			ayu.localGiftIdsValue(),
			ayu.localProfileEditorValue(),
			ayu.sentLocalGiftsValue()
		) | rpl::map([=](auto &&, const QString &, bool, auto &&) {
			const auto &settings = AyuSettings::getInstance();
			auto extra = 0;
			if (settings.localProfileEditor()) {
				if (peer->isSelf()) {
					extra = int(settings.localGiftRecords().size());
				} else {
					for (const auto &sent : settings.sentLocalGifts()) {
						if (sent.toId == peer->id.value) {
							++extra;
						}
					}
				}
			}
			return peer->peerGiftsCount() + extra;
		}) | rpl::type_erased;
	}
	return peer->session().changes().peerFlagsValue(
		peer,
