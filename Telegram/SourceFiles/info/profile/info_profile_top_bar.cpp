...
, _badgeContent(rpl::combine(
	BadgeContentForPeer(_peer),
	AyuSettings::getInstance().localProfileBackgroundGiftIdValue(),
	AyuSettings::getInstance().localGiftIdsValue()
) | rpl::map([=](Badge::Content content, quint64 id, const QString &) {
	if (id
		&& _peer->isSelf()
		&& AyuSettings::getInstance().localProfileEditor()
		&& AyuSettings::getInstance().isLocalGiftId(id)) {
		if (auto collectible = LocalProfileCollectible()) {
			_peer->session().data().emojiStatuses().registerLocalCollectible(
				collectible);
			content.badge = BadgeType::Premium;
			content.emojiStatusId = EmojiStatusId{
				.collectible = std::move(collectible),
			};
		}
	}
	return content;
...
, _backToggles(std::move(descriptor.backToggles)) {
	AyuSettings::getInstance().localProfileBackgroundGiftIdValue(
	) | rpl::on_next([=, controller = descriptor.controller] {
	...
	AyuSettings::getInstance().localHiddenGiftIdsValue(
	) | rpl::on_next([=, controller = descriptor.controller] {
		if (_pinnedToTopGiftsFirstTimeShowed) {
	...
	if (_badge) {
		...
		_badge->setPremiumClickCallback([controller, peer = _peer] {
			const auto id = AyuSettings::getInstance()
				.localProfileBackgroundGiftId();
			if (peer->isSelf()
				&& AyuSettings::getInstance().localProfileEditor()
				&& id
				&& AyuSettings::getInstance().isLocalGiftId(id)) {
				ShowLocalGiftInfo(controller, id);
			} else {
...
void TopBar::setupButtons(
	...
	rpl::combine(
		...
		if (wrap != Wrap::Side) {
			if (source == Source::Stories || (_peer->isSelf()
				&& AyuSettings::getInstance().localProfileEditor())) {
...
void TopBar::addTopBarEditButton(
	...
	_topBarButton->addClickHandler([=] {
		if (_peer->isSelf()
			&& AyuSettings::getInstance().localProfileEditor()) {
			ShowLocalProfileEditor(controller);
		} else {
...
void TopBar::setupNewGifts(
	...
	LOG(("AyuGiftsRow: setupNewGifts received=%1 filtered=%2").arg(gifts.size()).arg(filteredGifts.size()));
