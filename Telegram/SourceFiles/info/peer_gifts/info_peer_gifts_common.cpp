...
true));
		...
		_price.setMarkedText(
			...
			kMarkupTextOptions,
			_delegate->textContext());
if (ayu.isLocalGiftId(data.info.id)) {
			const auto localTitle = ayu.localGiftTitle(data.info.id);
			if (!localTitle.isEmpty()) {
				_text.setMarkedText(st::defaultTextStyle, tr::marked(localTitle));
			}
			const auto localPrice = ayu.localGiftPrice(data.info.id);
			if (localPrice > 0) {
				_price.setMarkedText(
					st::semiboldTextStyle,
					_delegate->star().append(' ' + format(localPrice)),
					kMarkupTextOptions,
					_delegate->textContext());
			}
		}
