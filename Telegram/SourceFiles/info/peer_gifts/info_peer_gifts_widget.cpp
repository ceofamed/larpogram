...
, _scrollAnimation([=] { updateScrollCallback(); }) {
	...
	AyuSettings::getInstance().localGiftIdsValue(
	) | rpl::on_next([=] {
		refreshLocalGifts();
	}, lifetime());

	AyuSettings::getInstance().localPinnedGiftIdsValue(
	) | rpl::on_next([=] {
		refreshLocalGifts();
	}, lifetime());

	AyuSettings::getInstance().sentLocalGiftsValue(
	) | rpl::on_next([=] {
		refreshLocalGifts();
	}, lifetime());

	AyuSettings::getInstance().localCollectionGiftIdsValue(
	) | rpl::on_next([=] {
		refreshLocalGifts();
	}, lifetime());

	_descriptor.value(
	) | rpl::map([](const Descriptor &d) {
		return d.filter;
	}) | rpl::distinct_until_changed()
	| rpl::filter([](const Filter &) { return true; })
	| rpl::on_next([=] {
		crl::on_main(this, [=] { refreshLocalGifts(); });
	}, lifetime());
...
void InnerWidget::loaded(const MTPpayments_SavedStarGifts &result) {
	...
	// Append local gifts from AyuGram settings.
	refreshLocalGifts();
...
void InnerWidget::refreshLocalGifts() {
	...
	const auto mine = _peer->isSelf();
	const auto sent = mine
		? std::vector<AyuSettings::SentLocalGift>()
		: ayu.sentLocalGifts();
	// Remove previously appended ayu gifts wherever they are.
	for (auto i = _list->begin(); i != _list->end();) {
		const auto id = i->gift.info.id;
		const auto foreign = ranges::contains(sent, id, [](const auto &s) {
			return s.record.id;
		});
		if (ayu.isLocalGiftId(id) || foreign) {
			i = _list->erase(i);
		} else {
			++i;
		}
	...
	// Re-append current local gifts at the very top:
	// pinned ones first (in their order), then the rest, newest first.
	auto fallbackDoc = static_cast<DocumentData*>(nullptr);
	if (!_list->empty()) {
		fallbackDoc = _list->front().gift.info.document;
	}
	const auto descriptor = _descriptor.current();
	const auto filter = descriptor.filter;
	const auto collectionId = descriptor.collectionId;
	const auto selectionId = _addingToCollectionId;
	const auto activeCollectionId = selectionId ? selectionId : collectionId;
	const auto localVisible = [&](const AyuSettings::LocalGiftRecord &r) {
		if (filter.skipUnique || filter.skipLimited || filter.skipSaved) {
			return false;
		}
		if (!selectionId && activeCollectionId
			&& !ayu.isLocalGiftInCollection(r.id, activeCollectionId)) {
			return false;
		}
		return true;
	};
	const auto localManageId = [](quint64 id) {
		return Data::SavedStarGiftId::User(MsgId(int64(id)));
	};
	const auto localMembership = [&](const AyuSettings::LocalGiftRecord &r) {
		return ayu.isLocalGiftInCollection(r.id, activeCollectionId);
	};
	if (!mine) {
		EnsureLocalTransferMessages(_peer, crl::guard(this, [=] {
			refreshLocalGifts();
		}));
	}
	const auto records = mine ? ayu.localGiftRecords() : std::vector<AyuSettings::LocalGiftRecord>();
	auto preloadRecords = records;
	if (!mine) {
		auto seen = base::flat_set<quint64>();
		for (const auto &record : records) {
			seen.emplace(record.id);
		}
		for (const auto &s : ayu.sentLocalGifts()) {
			if (s.toId == _peer->id.value
				&& seen.emplace(s.record.id).second) {
				preloadRecords.push_back(s.record);
			}
		}
	}
	if (PreloadLocalGiftDocuments(
			&_peer->session(),
			preloadRecords,
			crl::guard(this, [=] {
				refreshLocalGifts();
	...
	auto pinnedCount = 0;
	const auto pinnedIds = mine
		? ayu.localPinnedGiftIds().split(',', Qt::SkipEmptyParts)
		: QStringList();
	for (auto pit = pinnedIds.rbegin(); pit != pinnedIds.rend(); ++pit) {
		const auto id = pit->trimmed().toULongLong();
		const auto it = ranges::find(records, id, &AyuSettings::LocalGiftRecord::id);
		if (it == end(records) || !localVisible(*it)) {
			continue;
		}
		auto saved = LocalGiftFromRecord(_peer, *it, fallbackDoc);
		if (!saved) {
			continue;
		}
		saved->pinned = true;
		if (localMembership(*it)) {
			saved->collectionIds.push_back(activeCollectionId);
		}
		auto desc = DescriptorForGift(_peer, *saved);
	...
	auto rest = std::vector<AyuSettings::LocalGiftRecord>();
	for (const auto &record : records) {
		if (ayu.isLocalGiftPinned(record.id)
			|| !localVisible(record)) {
			continue;
		}
		rest.push_back(record);
	}
	if (filter.sortByValue) {
		ranges::sort(rest, ranges::greater(), [](const auto &r) {
			return r.valueAmount ? r.valueAmount : r.price;
		});
	}
	for (const auto &record : rest) {
		auto saved = LocalGiftFromRecord(_peer, record, fallbackDoc);
		if (!saved) {
			continue;
		}
		if (localMembership(record)) {
			saved->collectionIds.push_back(activeCollectionId);
		}
		auto desc = DescriptorForGift(_peer, *saved);
		_list->insert(_list->begin() + pinnedCount, {
			.gift = std::move(*saved),
			.descriptor = std::move(desc),
		});
		++_localGiftCount;
	...
	if (!mine) {
		auto afterPinnedIndex = 0;
		for (auto i = _list->begin(); i != _list->end(); ++i) {
			if (i->gift.pinned
				&& !ayu.isLocalGiftId(i->gift.info.id)
				&& !ranges::contains(sent, i->gift.info.id, [](const auto &s) {
					return s.record.id;
				})) {
				afterPinnedIndex = int(i - _list->begin()) + 1;
			}
		}
		auto foreign = std::vector<AyuSettings::SentLocalGift>();
		for (const auto &s : sent) {
			if (s.toId != _peer->id.value) {
				continue;
			}
			if (!localVisible(s.record)) {
				continue;
			}
			foreign.push_back(s);
		}
		ranges::sort(foreign, ranges::greater(), [&](const auto &s) {
			return filter.sortByValue
				? (s.record.valueAmount
					? s.record.valueAmount
					: s.record.price)
				: s.record.date;
		});
		auto offset = afterPinnedIndex;
		for (auto it = foreign.begin(); it != foreign.end(); ++it) {
			...
			if (localMembership(it->record)) {
				saved->collectionIds.push_back(activeCollectionId);
			}
			auto desc = DescriptorForGift(_peer, *saved);
			_list->insert(_list->begin() + offset, {
				.gift = std::move(*saved),
				.descriptor = std::move(desc),
			});
			++offset;
	...
	if (activeCollectionId) {
		if (mine) {
			for (const auto &record : records) {
				if (localMembership(record)) {
					_inCollection.emplace(localManageId(record.id));
				}
			}
		} else {
			for (const auto &s : sent) {
				if (ayu.isLocalGiftInCollection(s.record.id, activeCollectionId)) {
					_inCollection.emplace(localManageId(s.record.id));
				}
			}
		}
	}

	_entries->total = _list->size();
	refreshButtons();
	refreshAbout();
}
...
void InnerWidget::addGiftToCollection(
	...
	const auto localId = (giftId.isUser()
		&& ayu.isLocalGiftId((quint64)giftId.userMessageId().bare))
		? (quint64)giftId.userMessageId().bare
		: 0;
	if (localId) {
		ayu.setLocalGiftInCollection(localId, collectionId, true);
		if (_addingToCollectionId == collectionId) {
			auto currentChanges = _collectionChanges.current();
			if (!ranges::contains(currentChanges.added, giftId)) {
...
void InnerWidget::showMenuFor(not_null<GiftButton*> button, QPoint point) {
	...
	if (AyuSettings::getInstance().isLocalGiftId(gift.info.id)) {
		const auto id = gift.info.id;
		const auto addAction = Ui::Menu::CreateAddActionCallback(_menu);
		addAction(Ui::Menu::MenuCallback::Args{
			.text = u"Изменить подарок"_q,
			.handler = [=] {
				ShowEditLocalGiftAttributes(_window, id, [=] {
					refreshLocalGifts();
				});
			},
...
void InnerWidget::editCollectionGifts(int id) {
	...
	_window->uiShow()->show(Box([=](not_null<Ui::GenericBox*> box) {
		...
		box->addButton(std::move(text), [=] {
			...
			auto addLocal = QVector<quint64>();
			auto removeLocal = QVector<quint64>();
			const auto &changes = state->changes.current();
			auto &ayu = AyuSettings::getInstance();
			for (const auto &id : changes.added) {
				if (id.isUser()
					&& ayu.isLocalGiftId((quint64)id.userMessageId().bare)) {
					addLocal.push_back((quint64)id.userMessageId().bare);
				} else {
			...
			for (const auto &id : changes.removed) {
				if (id.isUser()
					&& ayu.isLocalGiftId((quint64)id.userMessageId().bare)) {
					removeLocal.push_back((quint64)id.userMessageId().bare);
				} else {
					remove.push_back(Api::InputSavedStarGiftId(id));
				}
			}
			for (const auto &giftId : addLocal) {
				ayu.setLocalGiftInCollection(giftId, id, true);
			}
			for (const auto &giftId : removeLocal) {
				ayu.setLocalGiftInCollection(giftId, id, false);
			}
	...
	for (const auto &collection : _collections) {
		...
		if (collection.icon) {
			...
			for (const auto &record : ayu.localGiftRecords()) {
				if (!ayu.isLocalGiftInCollection(
					record.id,
					collection.id)
					|| !record.documentId) {
					continue;
				}
				if (const auto doc = _window->session().data().document(
					record.documentId).get(); doc->sticker()) {
					title.append(
						Data::SingleCustomEmoji(doc)
					).append(' ');
				}
				break;
			}
...
void InnerWidget::removeGiftFromCollection(
	...
	const auto localId = (giftId.isUser()
		...
		: 0;
	if (localId) {
		ayu.setLocalGiftInCollection(localId, collectionId, false);
		if (_addingToCollectionId == collectionId) {
			auto currentChanges = _collectionChanges.current();
			if (!ranges::contains(currentChanges.removed, giftId)) {
...
void InnerWidget::requestReorder(int fromIndex, int toIndex) {
	...
	const auto &ayu = AyuSettings::getInstance();
	if (_peer->isSelf()
		&& ayu.localProfileEditor()
		&& fromIndex < _list->size()
		&& ayu.isLocalGiftId((*_list)[fromIndex].gift.info.id)) {
		auto ids = QStringList();
		for (const auto &entry : *_list) {
			if (entry.gift.pinned
				&& ayu.isLocalGiftId(entry.gift.info.id)) {
				ids.push_back(QString::number(entry.gift.info.id));
			}
		}
		AyuSettings::getInstance().setLocalPinnedGiftIds(ids.join(','));
		return;
	}
	const auto collectionId = _descriptor.current().collectionId;
	if (collectionId) {
