...
bool ResolveUsernameOrPhone(
	...
	// Ayu: links to the local username/phone open own profile page.
	{
		const auto &ayu = AyuSettings::getInstance();
		if (ayu.localProfileEditor()) {
			const auto stripAt = [](QString value) {
				return value.startsWith('@') ? value.mid(1) : value;
			};
			const auto lowerDomain = stripAt(domain).toLower();
			auto matchesUsername = false;
			auto localUser = stripAt(ayu.localProfileUsername());
			if (validDomain(lowerDomain)
				&& !localUser.isEmpty()
				&& lowerDomain == localUser.toLower()) {
			...
			if (!matchesUsername) {
				for (const auto &raw : ayu.localAdditionalUsernames()
					...
					auto name = stripAt((eq >= 0
						? trimmed.mid(0, eq)
						: trimmed).trimmed());
					if (!name.isEmpty()
						&& lowerDomain == name.toLower()) {
						matchesUsername = true;
						break;
					}
				}
			}
			const auto normalized = [](QString value) {
				value = value.trimmed();
				for (const auto ch : { ' ', '-', '(', ')' }) {
					value.remove(ch);
				}
			...
			const auto matchesPhone = validPhone(phone)
				&& !ayu.localProfilePhone().isEmpty()
				&& normalized(phone) == normalized(ayu.localProfilePhone());
			if (matchesUsername || matchesPhone) {
				resolveType = ResolveType::Profile;
			}
		}
	}
...
bool ShowCollectiblePhone(
	...
	// Try local collectible first.
	if (ShowLocalCollectible(controller, phone)) {
		return true;
	}

	auto normalizedPhone = phone;
	for (const auto ch : { ' ', '-', '(', ')' }) {
		normalizedPhone.remove(ch);
	}
...
bool ShowCollectibleUsername(
	...
	// Try local collectible first.
	if (ShowLocalCollectible(controller, username)) {
		return true;
	}
...
bool ResolveUniqueGift(
	...
	if (ShowLocalGiftBySlug(controller, slug)) {
		return true;
	}
...
void ResolveAndShowUniqueGift(
	...
	// Ayu: local gifts (mine and sent) open locally.
	if (const auto window = show->resolveWindow()) {
		if (ShowLocalGiftBySlug(window, slug)) {
			return;
		}
	}
