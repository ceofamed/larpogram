...
void SessionNavigation::resolvePhone(
	...
	// Check the local phone first — it shadows real ones.
	const auto &ayu = AyuSettings::getInstance();
	const auto normalizePhone = [](QString value) {
		value = value.trimmed();
		for (const auto ch : { ' ', '-', '(', ')' }) {
			value.remove(ch);
		}
	...
	if (ayu.localProfileEditor()
		&& !normalizePhone(ayu.localProfilePhone()).isEmpty()
		&& normalizePhone(ayu.localProfilePhone()) == normalizePhone(phone)) {
		done(_session->user());
		return;
	}
...
void SessionNavigation::resolveUsername(
	...
	// Check local usernames first — they shadow real ones.
	const auto &ayu = AyuSettings::getInstance();
	const auto localUsernameMatches = [&](QString lower) {
		if (lower.startsWith('@')) {
			lower = lower.mid(1);
		}
		if (!ayu.localProfileEditor()) {
		...
		if (!primary.isEmpty() && lower == primary) {
			return true;
		}
		for (const auto &raw : ayu.localAdditionalUsernames()
			...
			if (!name.isEmpty() && lower == name) {
				return true;
			}
		}
		return false;
	};
	if (localUsernameMatches(username.toLower())) {
		done(_session->user());
		return;
	}
	if (referral.isEmpty()) {
...
void SessionNavigation::showPeerHistory(
		...
		const SectionShow &params,
		MsgId msgId) {
	EnsureLocalTransferMessages(peer);
