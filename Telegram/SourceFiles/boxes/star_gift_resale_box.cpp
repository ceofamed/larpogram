...
void AddResaleGiftsList(
	...
	if (bought) {
		...
		customHandler = crl::guard(container, [=](Descriptor descriptor) {
			...
			const auto &starGift = v::get<StarGift>(descriptor);
			const auto unique = starGift.info.unique;
			const auto info = starGift.info;
			const auto done = crl::guard(container, [=](bool ok) {
				if (ok) {
					bought(unique);
					// Record locally.
					AyuSettings::getInstance().addLocalGift(
						MakeLocalGiftRecord(info,
					unique->starsForResale > 0 ? unique->starsForResale : 0));
					window->showToast({
						.text = TextWithEntities{
							u"🎁 Подарок добавлен в ваш профиль"_q },
						.iconLottie = u"gift"_q,
					});
				}
			});
			if (AyuSettings::getInstance().localProfileEditor()) {
				done(true);
				return;
			}
