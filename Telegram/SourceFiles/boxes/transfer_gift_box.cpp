...
void TransferGift(
	...
	// Ayu: local gifts transfer locally.
	const auto transferPrice = int64(25);
	LOG(("AyuGiftsTransfer: called giftId=%1").arg(gift ? gift->id : 0));
	if (AyuSettings::getInstance().localProfileEditor() && gift) {
		if (const auto record = AyuSettings::getInstance().localGiftRecord(gift->id)) {
			LOG(("AyuGiftsTransfer: local transfer id=%1 to=%2").arg(record->id).arg(to->id.value));
			auto &settings = AyuSettings::getInstance();
			settings.setLocalStarsBalance(std::max(int64(0), settings.localStarsBalance() - transferPrice));
			settings.addSentLocalGift(*record, to->id.value);
			settings.removeLocalGift(record->id);
			AddLocalTransferMessage(&window->session(), to, *record);
