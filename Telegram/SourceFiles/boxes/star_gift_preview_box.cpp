...
void StarGiftPreviewBox(
		...
		Fn<void(Data::UniqueGift)> applied) {
	...
	box->addTopButton(st::uniqueCloseButton, [=] { box->closeBox(); });
	if (applied) {
		box->addTopButton(st::uniqueApplyButton, [=] {
			applied(state->gift.current().values);
			box->closeBox();
		});
	}
