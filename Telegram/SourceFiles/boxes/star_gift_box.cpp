...
void AddUpgradeGiftCover(
	...
	const auto showAll = [=, list = args.all] {
		...
		show->show(Box(StarGiftPreviewBox, title, list, type, null, nullptr));
	};
...
void DefaultGiftHandler(
	...
	// Hook for local gifts owned by the user.
	if (star && star->mine && peer->isSelf()
		&& AyuSettings::getInstance().isLocalGiftId(star->info.id)) {
		ShowLocalGiftInfo(window, star->info.id);
		return;
	}
	// Hook for local purchase of resale gifts from the marketplace.
	if (unique && star->resale
		&& AyuSettings::getInstance().localProfileEditor()
		&& peer->isSelf()) {
		AyuSettings::getInstance().addLocalGift(
			MakeLocalGiftRecord(star->info,
				unique->starsForResale > 0 ? unique->starsForResale : 0));
		const auto title = !unique->title.isEmpty()
			? unique->title
			: (u"#%1"_q.arg(unique->id));
		window->showToast({
			.text = TextWithEntities{
				u"🎁 Подарок "_q + title + u" добавлен в ваш профиль"_q },
			.iconLottie = u"gift"_q,
		});
		return;
	}
	if (unique && star->resale) {
...
if (stars && AyuSettings::getInstance().localProfileEditor() && peer->isSelf()) {
			AyuSettings::getInstance().addLocalGift(
				MakeLocalGiftRecord(stars->info, stars->info.stars));
			const auto title = !stars->info.resellTitle.isEmpty()
				? stars->info.resellTitle
				: (u"#%1"_q.arg(stars->info.id));
			window->showToast({
				.text = TextWithEntities{
					u"🎁 Подарок "_q + title + u" добавлен в ваш профиль"_q },
				.iconLottie = u"gift"_q,
			...
			window->showToast(u"Ошибка отправки: %1"_q.arg(
