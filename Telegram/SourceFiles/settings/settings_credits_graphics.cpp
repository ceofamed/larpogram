...
void ToggleStarGiftSaved(
	...
	const auto giftId = savedId.userMessageId().bare;
	if (giftId > 0 && AyuSettings::getInstance().isLocalGiftId(
			quint64(giftId))) {
		AyuSettings::getInstance().setLocalGiftHiddenFromProfile(
			quint64(giftId),
			!save);
...
void FillUniqueGiftMenu(
	...
	// Ayu: actions for locally added gifts.
	const auto &ayu = AyuSettings::getInstance();
	if (const auto unique = e.uniqueGift) {
		if (ayu.localProfileEditor()
			&& !e.giftTransferred
			&& ayu.isLocalGiftId(e.stargiftId)) {
			const auto giftId = e.stargiftId;
			if (const auto window = show->resolveWindow()) {
				const auto slug = unique->slug;
				if (!slug.isEmpty()) {
					const auto link = u"https://t.me/nft/"_q + slug;
					menu->addAction(tr::lng_context_copy_link(tr::now), [=] {
						QGuiApplication::clipboard()->setText(link);
					});
					menu->addAction(tr::lng_share_title(tr::now), [=] {
						FastShareLink(window, link);
					});
				}
				if (const auto record = ayu.localGiftRecord(giftId)) {
					menu->addAction(u"Передать подарок"_q, [=] {
						ShowTransferLocalGift(window, *record);
					});
				}
				menu->addAction(
					ayu.isLocalGiftPinned(giftId)
						? QString(u"Открепить от профиля"_q)
						: QString(u"Закрепить в профиле"_q),
					[=] {
						auto &settings = AyuSettings::getInstance();
						settings.setLocalGiftPinned(
							giftId,
							!settings.isLocalGiftPinned(giftId));
					});
				menu->addAction(u"Изменить цену"_q, [=] {
					ShowEditLocalGiftPrice(window, giftId);
				});
				menu->addAction(u"Изменить подарок"_q, [=] {
					ShowEditLocalGiftAttributes(window, giftId);
				});
				menu->addAction(
					(ayu.localProfileBackgroundGiftId() == giftId)
						? QString(u"Убрать фон профиля"_q)
						: QString(u"Фон профиля"_q),
					[=] {
						auto &settings = AyuSettings::getInstance();
						settings.setLocalProfileBackgroundGiftId(
							settings.localProfileBackgroundGiftId() == giftId
								? quint64(0)
								: giftId);
					});
				menu->addAction(u"Убрать из профиля"_q, [=] {
					AyuSettings::getInstance().removeLocalGift(giftId);
					AyuSettings::getInstance()
						.setLocalProfileBackgroundGiftId(0);
					if (window) {
						window->hideLayer(anim::type::normal);
					}
				});
			}
			return;
