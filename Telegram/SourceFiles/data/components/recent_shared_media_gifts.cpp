...
void RecentSharedMediaGifts::request(
		...
		bool onlyPinnedToTop) {
	const auto finish = [=](std::vector<SavedStarGift> gifts) {
		const auto &ayu = AyuSettings::getInstance();
		LOG(("AyuGiftsRow: finish server=%1 onlyPinned=%2").arg(gifts.size()).arg(onlyPinnedToTop));
		if (peer->isSelf()
			&& onlyPinnedToTop
			&& ayu.localProfileEditor()) {
			auto locals = std::vector<SavedStarGift>();
			auto missing = std::vector<QString>();
			for (const auto &record : ayu.localGiftRecords()) {
				if (!ayu.isLocalGiftPinned(record.id)
					|| ayu.isLocalGiftHiddenFromProfile(record.id)) {
					continue;
				}
				auto doc = record.documentId
					...
					: nullptr;
				if (doc && doc->sticker()) {
					if (auto gift = LocalGiftFromRecord(
							peer,
							record,
							nullptr)) {
						gift->pinned = true;
						locals.push_back(std::move(*gift));
					}
				} else if (!record.slug.isEmpty()) {
					missing.push_back(record.slug);
				}
			}
			LOG(("AyuGiftsRow: locals=%1 missing=%2").arg(locals.size()).arg(missing.size()));
			if (!missing.empty()) {
				struct FetchState {
					int remaining = 0;
					std::vector<SavedStarGift> gifts;
					std::vector<SavedStarGift> locals;
				};
				const auto session = &peer->session();
				const auto state = std::make_shared<FetchState>();
				state->remaining = int(missing.size());
				state->gifts = std::move(gifts);
				state->locals = std::move(locals);
				const auto oneDone
					= [=](std::optional<Data::StarGift> parsed) {
						if (parsed) {
							...
							state->locals.push_back(std::move(saved));
						}
						if (--state->remaining == 0) {
							auto all = std::move(state->locals);
							for (auto &gift : state->gifts) {
								all.push_back(std::move(gift));
							}
							done(std::move(all));
						}
					};
				for (const auto &slug : missing) {
					session->api().request(
						MTPpayments_GetUniqueStarGift(MTP_string(slug))
					).done([=](const MTPpayments_UniqueStarGift &result) {
						session->data().processUsers(
							result.data().vusers());
						Api::FromTL(session, result.data().vgift());
						const auto &ayuNow = AyuSettings::getInstance();
						for (const auto &record : ayuNow.localGiftRecords()) {
							if (record.slug.isEmpty()
								|| record.slug.toLower() != slug.toLower()
								|| ayuNow.isLocalGiftHiddenFromProfile(
									record.id)) {
								continue;
							}
							if (auto gift = LocalGiftFromRecord(
									...
									nullptr)) {
								gift->pinned = true;
								state->locals.push_back(std::move(*gift));
							}
							break;
						}
						oneDone(std::nullopt);
	...
	if (it != _recent.end()) {
		...
		if (entry.lastRequestTime
			...
			finish(filterGifts(entry.gifts, onlyPinnedToTop));
			return;
		}
		if (entry.requestId) {
			entry.pendingCallbacks.push_back([=] {
				const auto it = _recent.find(peer->id);
				if (it != _recent.end()) {
					finish(filterGifts(it->second.gifts, onlyPinnedToTop));
				}
			});
			return;
		}
