...
CreditsAmount Credits::balance() const {
	...
	if (ayu.localProfileEditor()) {
		return CreditsAmount(ayu.localStarsBalance());
	}
...
rpl::producer<CreditsAmount> Credits::balanceValue() const {
	const auto &ayu = AyuSettings::getInstance();
	if (ayu.localProfileEditor()) {
		return ayu.localStarsBalanceValue()
			| rpl::map([](int64 value) { return CreditsAmount(value); })
			| rpl::type_erased;
	}
...
void Credits::withdrawLocked(CreditsAmount count) {
	if (AyuSettings::getInstance().localProfileEditor()) {
		...
		settings.setLocalStarsBalance(
			std::max(
				int64(0),
				settings.localStarsBalance() - count.whole()));
		return;
	}
	Expects(count >= CreditsAmount(0));
	Expects(_locked >= count);
