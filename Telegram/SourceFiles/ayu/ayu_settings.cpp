...
void AyuSettings::setLocalProfileEditor(bool val) {
	if (_localProfileEditor.current() == val) return;
	_localProfileEditor = val;
	save();
}

void AyuSettings::setLocalProfileName(const QString &value) {
	if (_localProfileName.current() == value) return;
	_localProfileName = value;
	save();
}

void AyuSettings::setLocalProfileUsername(const QString &value) {
	if (_localProfileUsername.current() == value) return;
	_localProfileUsername = value;
	save();
}

void AyuSettings::setLocalProfilePhone(const QString &value) {
	if (_localProfilePhone.current() == value) return;
	_localProfilePhone = value;
	save();
}

void AyuSettings::setLocalAdditionalUsernames(const QString &value) {
	if (_localAdditionalUsernames.current() == value) return;
	_localAdditionalUsernames = value;
	save();
}

void AyuSettings::setLocalUsernamePrice(const QString &value) {
	if (_localUsernamePrice.current() == value) return;
	_localUsernamePrice = value;
	save();
}

void AyuSettings::setLocalUsernameDate(const QString &value) {
	if (_localUsernameDate.current() == value) return;
	_localUsernameDate = value;
	save();
}

void AyuSettings::setLocalPhonePrice(const QString &value) {
	if (_localPhonePrice.current() == value) return;
	_localPhonePrice = value;
	save();
}

void AyuSettings::setLocalPhoneDate(const QString &value) {
	if (_localPhoneDate.current() == value) return;
	_localPhoneDate = value;
	save();
}

void AyuSettings::resetLocalProfile() {
	_localProfileName = QString();
	_localProfileUsername = QString();
	_localProfilePhone = QString();
	save();
}

void AyuSettings::resetLocalGifts() {
	_localGiftIds = QString();
	_localPinnedGiftIds = QString();
	_localCollectionGiftIds = QString();
	_localHiddenGiftIds = QString();
	save();
}

void AyuSettings::setLocalGiftIds(const QString &value) {
	if (_localGiftIds.current() == value) return;
	_localGiftIds = value;
	save();
}
...
[[nodiscard]] QString SerializeGiftAttributes(
		const AyuSettings::LocalGiftRecord &record) {
	auto j = nlohmann::json::object();
	j["mn"] = record.modelName.toStdString();
	j["mr"] = record.modelRarity;
	j["md"] = record.modelDocumentId;
	j["pn"] = record.patternName.toStdString();
	j["pr"] = record.patternRarity;
	j["pd"] = record.patternDocumentId;
	j["bn"] = record.backdropName.toStdString();
	j["br"] = record.backdropRarity;
	j["bc"] = record.backdropCenter.name(QColor::HexArgb).toStdString();
	j["be"] = record.backdropEdge.name(QColor::HexArgb).toStdString();
	j["bp"] = record.backdropPattern.name(QColor::HexArgb).toStdString();
	j["bt"] = record.backdropText.name(QColor::HexArgb).toStdString();
	j["bi"] = record.backdropId;
	j["num"] = record.number;
	j["igid"] = record.initialGiftId;
	j["slug"] = record.slug.toStdString();
	j["vc"] = record.valueCurrency.toStdString();
	j["va"] = record.valueAmount;
	j["vu"] = record.valueUsd;
	j["ll"] = record.limitedLeft;
	j["lc"] = record.limitedCount;
	const auto bytes = QByteArray::fromStdString(j.dump());
	return u"a:"_q + QString::fromLatin1(bytes.toBase64(
		QByteArray::Base64UrlEncoding | QByteArray::OmitTrailingEquals));
}

void ParseGiftAttributes(
		const QString &encoded,
		AyuSettings::LocalGiftRecord &record) {
	const auto bytes = QByteArray::fromBase64(
		encoded.mid(2).toLatin1(),
	...
	const auto string = [&](const char *key) {
		...
		return (i != j.end() && i->is_string())
			? QString::fromStdString(i->get<std::string>())
			: QString();
	};
	const auto integer = [&](const char *key) {
		const auto i = j.find(key);
		return (i != j.end() && i->is_number_integer())
			? i->get<int>()
			: 0;
	};
	const auto unsignedValue = [&](const char *key) {
		const auto i = j.find(key);
		return (i != j.end() && i->is_number_unsigned())
			? i->get<quint64>()
			: quint64(0);
	};
	const auto color = [&](const char *key) {
		return QColor(string(key));
	};
	const auto int64Value = [&](const char *key) {
		const auto i = j.find(key);
		return (i != j.end() && i->is_number_integer())
			? i->get<int64>()
			: int64(0);
	};
	record.modelName = string("mn");
	record.modelRarity = integer("mr");
	record.modelDocumentId = unsignedValue("md");
	record.patternName = string("pn");
	record.patternRarity = integer("pr");
	record.patternDocumentId = unsignedValue("pd");
	record.backdropName = string("bn");
	record.backdropRarity = integer("br");
	record.backdropCenter = color("bc");
	record.backdropEdge = color("be");
	record.backdropPattern = color("bp");
	record.backdropText = color("bt");
	record.backdropId = integer("bi");
	record.number = integer("num");
	record.slug = string("slug");
	record.initialGiftId = unsignedValue("igid");
	record.valueCurrency = string("vc");
	record.valueAmount = int64Value("va");
	record.valueUsd = int64Value("vu");
	record.limitedLeft = integer("ll");
	record.limitedCount = integer("lc");
}

[[nodiscard]] QString SerializeGiftDocRemotes(
		const AyuSettings::LocalGiftRecord &record) {
	auto j = nlohmann::json::object();
	j["maa"] = record.modelAccess;
	j["mrf"] = record.modelFileReference.toBase64().toStdString();
	j["mdc"] = record.modelDc;
	j["msz"] = record.modelSize;
	j["mdt"] = record.modelDate;
	j["mm"] = record.modelMime.toStdString();
	j["mal"] = record.modelAlt.toStdString();
	j["paa"] = record.patternAccess;
	j["prf"] = record.patternFileReference.toBase64().toStdString();
	j["pdc"] = record.patternDc;
	j["psz"] = record.patternSize;
	j["pdt"] = record.patternDate;
	j["pm"] = record.patternMime.toStdString();
	j["pal"] = record.patternAlt.toStdString();
	const auto bytes = QByteArray::fromStdString(j.dump());
	return u"r:"_q + QString::fromLatin1(bytes.toBase64(
		QByteArray::Base64UrlEncoding | QByteArray::OmitTrailingEquals));
}

void ParseGiftDocRemotes(
		const QString &encoded,
		AyuSettings::LocalGiftRecord &record) {
	const auto bytes = QByteArray::fromBase64(
		encoded.mid(2).toLatin1(),
	...
	const auto base64Bytes = [&](const char *key) {
		return QByteArray::fromBase64(
			string(key).toLatin1(),
			QByteArray::AbortOnBase64DecodingErrors);
	};
	record.modelAccess = unsignedValue("maa");
	record.modelFileReference = base64Bytes("mrf");
	record.modelDc = integer("mdc");
	record.modelSize = int64Value("msz");
	record.modelDate = integer("mdt");
	record.modelMime = string("mm");
	record.modelAlt = string("mal");
	record.patternAccess = unsignedValue("paa");
	record.patternFileReference = base64Bytes("prf");
	record.patternDc = integer("pdc");
	record.patternSize = int64Value("psz");
	record.patternDate = integer("pdt");
	record.patternMime = string("pm");
	record.patternAlt = string("pal");
}

QString SerializeGiftEntry(const AyuSettings::LocalGiftRecord &record) {
	auto result = QString::number(record.id)
		+ '='
		+ QString::number(record.price)
		+ '='
		+ QString::number(record.date);
	if (record.documentId) {
		result += "=d:" + QString::number(record.documentId);
	}
	if (record.hasAppearanceData()) {
		result += '=' + SerializeGiftAttributes(record);
	}
	if (record.modelAccess || record.patternAccess) {
		result += '=' + SerializeGiftDocRemotes(record);
	}
...
std::optional<AyuSettings::LocalGiftRecord> ParseGiftEntry(
		const QString &entry) {
	const auto fields = entry.split('=', Qt::KeepEmptyParts);
	if (fields.isEmpty()) {
		return std::nullopt;
	}
	bool ok = false;
	auto record = AyuSettings::LocalGiftRecord();
	record.id = fields.front().toULongLong(&ok);
	if (!ok || !record.id) {
		return std::nullopt;
	}
	if (fields.size() > 1) {
		record.price = fields[1].toLongLong();
	}
	if (fields.size() > 2) {
		record.date = fields[2].toLongLong();
	}
	auto titleFieldsStart = 3;
	if (fields.size() > titleFieldsStart
		&& fields[titleFieldsStart].startsWith(u"d:"_q)) {
		record.documentId = fields[titleFieldsStart].mid(2).toULongLong();
		++titleFieldsStart;
	}
	if (fields.size() > titleFieldsStart
		&& fields[titleFieldsStart].startsWith(u"a:"_q)) {
		ParseGiftAttributes(fields[titleFieldsStart], record);
		++titleFieldsStart;
	}
	if (fields.size() > titleFieldsStart
		&& fields[titleFieldsStart].startsWith(u"r:"_q)) {
		ParseGiftDocRemotes(fields[titleFieldsStart], record);
		++titleFieldsStart;
	}
	if (fields.size() > titleFieldsStart) {
		record.title = QStringList(
			fields.begin() + titleFieldsStart,
			fields.end()).join('=');
	}
	return record;
}
...
void AyuSettings::addLocalGift(
		quint64 id,
		int64 price,
		int64 date,
		const QString &title,
		quint64 documentId) {
	auto record = LocalGiftRecord();
	record.id = id;
	record.price = price;
	record.date = date;
	record.title = title;
...
void AyuSettings::addLocalGift(const LocalGiftRecord &record) {
	auto entries = _localGiftIds.current().split(',', Qt::SkipEmptyParts);
	const auto serialized = SerializeGiftEntry(record);
	for (auto &entry : entries) {
		if (const auto parsed = ParseGiftEntry(entry); parsed && parsed->id == record.id) {
			entry = serialized;
			_localGiftIds = entries.join(',');
			save();
			return;
		}
	}
	entries.push_back(serialized);
	_localGiftIds = entries.join(',');
	save();
}

void AyuSettings::updateLocalGift(
		quint64 id,
		int64 price,
		int64 date,
		const QString &title) {
	auto record = localGiftRecord(id).value_or(LocalGiftRecord());
	record.id = id;
	record.price = price;
	record.date = date;
	record.title = title;
...
void AyuSettings::removeLocalGift(quint64 id) {
	setLocalGiftPinned(id, false);
	setLocalGiftHiddenFromProfile(id, false);
	auto entries = _localGiftIds.current().split(',', Qt::SkipEmptyParts);
	entries.erase(
		std::remove_if(entries.begin(), entries.end(), [&](const QString &entry) {
			const auto parsed = ParseGiftEntry(entry);
			return parsed && parsed->id == id;
		}),
		entries.end());
	setLocalGiftIds(entries.join(','));
}

int64 AyuSettings::localGiftPrice(quint64 id) const {
	for (const auto &entry : _localGiftIds.current().split(',', Qt::SkipEmptyParts)) {
		if (const auto parsed = ParseGiftEntry(entry); parsed && parsed->id == id) {
			return parsed->price;
		}
	}
	return 0;
...
int64 AyuSettings::localGiftDate(quint64 id) const {
	for (const auto &entry : _localGiftIds.current().split(',', Qt::SkipEmptyParts)) {
		if (const auto parsed = ParseGiftEntry(entry); parsed && parsed->id == id) {
			return parsed->date;
		}
...
QString AyuSettings::localGiftTitle(quint64 id) const {
	for (const auto &entry : _localGiftIds.current().split(',', Qt::SkipEmptyParts)) {
		if (const auto parsed = ParseGiftEntry(entry); parsed && parsed->id == id) {
			return parsed->title;
		}
...
std::vector<AyuSettings::LocalGiftRecord> AyuSettings::localGiftRecords() const {
	auto result = std::vector<LocalGiftRecord>();
	for (const auto &entry : _localGiftIds.current().split(',', Qt::SkipEmptyParts)) {
		if (const auto parsed = ParseGiftEntry(entry)) {
...
std::optional<AyuSettings::LocalGiftRecord> AyuSettings::localGiftRecord(quint64 id) const {
	for (const auto &record : localGiftRecords()) {
		if (record.id == id) {
			return record;
		}
	}
...
bool AyuSettings::isLocalGiftId(quint64 id) const {
	if (_localGiftIdCacheKey != _localGiftIds.current()) {
		_localGiftIdCacheKey = _localGiftIds.current();
		_localGiftIdCache.clear();
		const auto parts = _localGiftIdCacheKey.split(',', Qt::SkipEmptyParts);
		for (const auto &part : parts) {
			bool ok = false;
			const auto valueText = part.section('=', 0, 0);
			if (const auto value = valueText.toULongLong(&ok); ok) {
				_localGiftIdCache.insert(value);
			}
		}
	}
	return _localGiftIdCache.contains(id);
}

void AyuSettings::setLocalStarsBalance(int64 value) {
	if (_localStarsBalance.current() == value) return;
	_localStarsBalance = value;
	save();
}

void AyuSettings::setLocalProfileBackgroundGiftId(quint64 id) {
	if (_localProfileBackgroundGiftId.current() == id) return;
	_localProfileBackgroundGiftId = id;
	save();
}

bool AyuSettings::isLocalGiftPinned(quint64 id) const {
	const auto idText = QString::number(id);
	for (const auto &value : _localPinnedGiftIds.current().split(',', Qt::SkipEmptyParts)) {
		if (value.trimmed() == idText) {
			return true;
		}
	}
	return false;
...
void AyuSettings::setLocalGiftPinned(quint64 id, bool pinned) {
	const auto idText = QString::number(id);
	auto ids = _localPinnedGiftIds.current().split(',', Qt::SkipEmptyParts);
	ids.erase(
		std::remove_if(
			ids.begin(),
			ids.end(),
			[&](const QString &value) { return value.trimmed() == idText; }),
		ids.end());
	if (pinned) {
		ids.push_back(idText);
	}
	const auto joined = ids.join(',');
	if (_localPinnedGiftIds.current() == joined) return;
	_localPinnedGiftIds = joined;
	save();
}

void AyuSettings::setLocalPinnedGiftIds(const QString &value) {
	if (_localPinnedGiftIds.current() == value) return;
	_localPinnedGiftIds = value;
	save();
}

bool AyuSettings::isLocalGiftHiddenFromProfile(quint64 id) const {
	const auto idText = QString::number(id);
	for (const auto &value : _localHiddenGiftIds.current().split(',', Qt::SkipEmptyParts)) {
		if (value.trimmed() == idText) {
			return true;
		}
	}
...
void AyuSettings::setLocalGiftHiddenFromProfile(quint64 id, bool hidden) {
	const auto idText = QString::number(id);
	auto ids = _localHiddenGiftIds.current().split(',', Qt::SkipEmptyParts);
	ids.erase(
		std::remove_if(
			ids.begin(),
			ids.end(),
	...
	const auto joined = ids.join(',');
	if (_localHiddenGiftIds.current() == joined) return;
	_localHiddenGiftIds = joined;
	save();
}

bool AyuSettings::isLocalGiftInCollection(quint64 giftId, int collectionId) const {
	const auto prefix = QString::number(collectionId) + ':';
	const auto full = prefix + QString::number(giftId);
	for (const auto &entry : _localCollectionGiftIds.current().split(',', Qt::SkipEmptyParts)) {
		if (entry == full) {
			return true;
		}
	}
...
void AyuSettings::setLocalGiftInCollection(
		quint64 giftId,
		int collectionId,
		bool inCollection) {
	auto entries = _localCollectionGiftIds.current().split(',', Qt::SkipEmptyParts);
	const auto full = QString::number(collectionId) + ':' + QString::number(giftId);
	entries.erase(
		std::remove_if(
			entries.begin(),
			entries.end(),
			[&](const QString &entry) { return entry == full; }),
		entries.end());
	if (inCollection) {
		entries.push_back(full);
	}
	const auto joined = entries.join(',');
	if (_localCollectionGiftIds.current() == joined) return;
	_localCollectionGiftIds = joined;
	save();
}

void AyuSettings::addSentLocalGift(
		const LocalGiftRecord &record,
		quint64 toId) {
	auto line = QString::number(toId) + '|' + SerializeGiftEntry(record);
	auto lines = _sentLocalGifts.current().split('~~', Qt::SkipEmptyParts);
	lines.push_back(line);
	_sentLocalGifts = lines.join("~~");
	save();
}

void AyuSettings::setLocalTransferMsgId(quint64 recordId, int64 msgId) {
	auto lines = _localTransferMsgIds.current().split("~~", Qt::SkipEmptyParts);
	auto found = false;
	for (auto &line : lines) {
		const auto sep = line.indexOf('|');
		if (sep < 0) continue;
		if (line.mid(0, sep).toULongLong() == recordId) {
			line = QString::number(recordId) + '|' + QString::number(msgId);
			found = true;
			break;
		}
	}
	if (!found) {
		lines.push_back(
			QString::number(recordId) + '|' + QString::number(msgId));
	}
	_localTransferMsgIds = lines.join("~~");
	save();
}

int64 AyuSettings::localTransferMsgId(quint64 recordId) const {
	for (const auto &line : _localTransferMsgIds.current().split("~~", Qt::SkipEmptyParts)) {
		const auto sep = line.indexOf('|');
		if (sep < 0) continue;
		if (line.mid(0, sep).toULongLong() == recordId) {
			return line.mid(sep + 1).toLongLong();
		}
	}
	return 0;
}

std::vector<AyuSettings::SentLocalGift> AyuSettings::sentLocalGifts() const {
	auto result = std::vector<SentLocalGift>();
	for (const auto &line : _sentLocalGifts.current().split("~~", Qt::SkipEmptyParts)) {
		const auto sep = line.indexOf('|');
		if (sep < 0) continue;
		bool ok = false;
		const auto toId = line.mid(0, sep).toULongLong(&ok);
		if (!ok) continue;
		const auto record = ParseGiftEntry(line.mid(sep + 1));
		if (!record) continue;
		result.push_back({ toId, std::move(*record) });
	}
	return result;
}
...
void to_json(nlohmann::json &j, const AyuSettings &s) {
	...
	j = nlohmann::json{
		...
		{"localProfileEditor", s._localProfileEditor.current()},
		{"localProfileName", s._localProfileName.current()},
		{"localProfileUsername", s._localProfileUsername.current()},
		{"localProfilePhone", s._localProfilePhone.current()},
		{"localUsernamePrice", s._localUsernamePrice.current()},
		{"localUsernameDate", s._localUsernameDate.current()},
		{"localPhonePrice", s._localPhonePrice.current()},
		{"localPhoneDate", s._localPhoneDate.current()},
		{"localAdditionalUsernames", s._localAdditionalUsernames.current()},
{"localGiftIds", s._localGiftIds.current()},
		{"localStarsBalance", s._localStarsBalance.current()},
		{"localProfileBackgroundGiftId", s._localProfileBackgroundGiftId.current()},
		{"localPinnedGiftIds", s._localPinnedGiftIds.current()},
		{"sentLocalGifts", s._sentLocalGifts.current()},
		{"localTransferMsgIds", s._localTransferMsgIds.current()},
		{"localCollectionGiftIds", s._localCollectionGiftIds.current()},
		{"localHiddenGiftIds", s._localHiddenGiftIds.current()},
...
void from_json(const nlohmann::json &j, AyuSettings &s) {
	...
	s._localProfileEditor = j.value("localProfileEditor", defaults._localProfileEditor.current());
	s._localProfileName = j.value("localProfileName", defaults._localProfileName.current());
	s._localProfileUsername = j.value("localProfileUsername", defaults._localProfileUsername.current());
s._localProfilePhone = j.value("localProfilePhone", defaults._localProfilePhone.current());
	s._localUsernamePrice = j.value("localUsernamePrice", defaults._localUsernamePrice.current());
	s._localUsernameDate = j.value("localUsernameDate", defaults._localUsernameDate.current());
	s._localPhonePrice = j.value("localPhonePrice", defaults._localPhonePrice.current());
	s._localPhoneDate = j.value("localPhoneDate", defaults._localPhoneDate.current());
	s._localAdditionalUsernames = j.value("localAdditionalUsernames", defaults._localAdditionalUsernames.current());
s._localGiftIds = j.value("localGiftIds", defaults._localGiftIds.current());
	s._localStarsBalance = std::clamp(j.value("localStarsBalance", defaults._localStarsBalance.current()), int64(0), int64(2'000'000'000));
	s._localProfileBackgroundGiftId = j.value("localProfileBackgroundGiftId", defaults._localProfileBackgroundGiftId.current());
	s._localPinnedGiftIds = j.value("localPinnedGiftIds", defaults._localPinnedGiftIds.current());
	s._sentLocalGifts = j.value("sentLocalGifts", defaults._sentLocalGifts.current());
	s._localTransferMsgIds = j.value("localTransferMsgIds", defaults._localTransferMsgIds.current());
	s._localCollectionGiftIds = j.value("localCollectionGiftIds", defaults._localCollectionGiftIds.current());
	s._localHiddenGiftIds = j.value("localHiddenGiftIds", defaults._localHiddenGiftIds.current());
