...
void BuildOther(SectionBuilder &builder, AyuSectionBuilder &ayu) {
	...
	ayu.addSettingToggle({
		.id = u"ayu/localProfileEditor"_q,
		.title = rpl::single(QString(u"Локальный редактор профиля и подарков"_q)),
		.getter = &AyuSettings::localProfileEditor,
		.setter = &AyuSettings::setLocalProfileEditor,
		.keywords = { u"profile"_q, u"gifts"_q, u"username"_q, u"phone"_q },
	});
	ayu.addSettingToggle({
